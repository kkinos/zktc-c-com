...
fn gen(buf: &mut String, node: &Node) -> Result<()> {
    match &node.kind {
        ...
        NodeKind::Mul(n) => {
            ...
            // a0 * a1
            writeln!(buf, "  mov t1, a0")?;
            writeln!(buf, "  subi t0, t0, 1")?;
            writeln!(buf, "  beq t0, zero, 6")?;
            writeln!(buf, "  add a0, t1")?;
            writeln!(buf, "  jal zero, -6")?;

            // if a1 < 0 then a0 = -a0
            writeln!(buf, "  bge a1, zero, 12")?;
            writeln!(buf, "  lil t0, 0xffff@l")?;
            writeln!(buf, "  lih t1, 0xffff@h")?;
            writeln!(buf, "  or t0, t1")?;
            writeln!(buf, "  xor a0, t0")?;
            writeln!(buf, "  addi a0, a0, 1")?;
            writeln!(buf, "  push a0")?;
        }
        NodeKind::Div(n) => {
            gen(buf, &n.left)?;
